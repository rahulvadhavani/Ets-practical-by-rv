$(document).ready(function () {
    ...
    $("#add_data_modal").on('click', function () {
        ...
        $('#role_permission_table input[type="checkbox"]').prop('checked', false);
        $("#preview_div").hide();
        $('#project_btn').html(
    ...
    $(document).on('click', '.module_view_record', function () {
        ...
        $('#info_role_permission_table input[type="checkbox"]').prop('checked', false);
        $.ajax({
            type: 'GET',
            data: {
                id: id,
            ...
            success: function (response) {
                ...
                if (response.status) {
                    $.each(response.data, function (key, value) {
                        if (key == 'permissions') {
                            $.each(value, function (index, permission) {
                                // For each permission, check the corresponding checkbox
                                $(`#info_checkbox-${permission.id}`)
                                    .prop(
                                        'checked', true);
                            });
                        } else {
                ...
                if (response.status) {
                    $.each(response.data, function (key, value) {
                        if (key == 'permissions') {
                            ...
                            $.each(value, function (index, permission) {
                                // For each permission, check the corresponding checkbox
                                $(`#checkbox-${permission.id}`).prop(
                                    'checked', true);
                            });
                            // Handle module checkboxes
                            $('tr').each(function () {
                                var moduleCheckbox = $(this).find(
                                    '.module-checkbox');
                                var allPermissionsChecked = true;
                                $(this).find('.permission-checkbox')
                                    .each(function () {
                                        if (!$(this).prop(
                                            'checked')) {
                                ...
                                // Set the module checkbox based on permissions
                                moduleCheckbox.prop('checked',
                                    allPermissionsChecked);

                                // Bind the checkbox change event for modules
                                moduleCheckbox.on('change', function () {
                                    var moduleIndex = $(this)
                                        .closest('tr').index();
                                    var permissionsInModule = $(
                                        this).closest('tr')
                                        .find(
                                            '.permission-checkbox'
                                        ).toArray();
                                    permissionsInModule.forEach(
                                        function (
                                            permissionCheckbox
                                        ) {
                                            $(permissionCheckbox)
                            ...
                            // Handle the "Check All Permissions" functionality
                            $('#checkAllPermissions').prop('checked', false);
                            if ($('input[name="permissions[]"]:checked')
                                .length === $('input[name="permissions[]"]')
                                    .length) {
                                $('#checkAllPermissions').prop('checked', true);
                            }

                            // Handle the "Check All Permissions" checkbox change event
                            $('#checkAllPermissions').on('change', function () {
                                $('input[name="permissions[]"]').prop(
                                    'checked', $(this).prop(
                                        'checked'));
                                $('input.module-checkbox').prop(
                                    'checked', $(this).prop(
                                        'checked'));
                            });
                        } else {
    ...
    $('#checkAllPermissions').change(function () {
        ...
        $('.permission-checkbox, .module-checkbox').prop('checked', isChecked);
    });

    // Handle module-level checkboxes
    $('.module-checkbox').change(function () {
        var moduleIndex = $(this).attr('id').split('-')[1];
        var isChecked = $(this).prop('checked');
        $('.module-' + moduleIndex).prop('checked', isChecked);
    });

    // Handle individual permission checkboxes
    $('.permission-checkbox').change(function () {
        var moduleIndex = $(this).attr('class').split(' ')[2].split('-')[1];
        var allChecked = $('.module-' + moduleIndex).length === $('.module-' + moduleIndex +
            ':checked').length;
        $('#module-' + moduleIndex).prop('checked', allChecked);

        // Update 'Check All' checkbox status
        var allPermissionsChecked = $('.permission-checkbox').length === $(
